...
class FacebookSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.start_urls = ['https://mbasic.facebook.com']    #Ruta inicial

    def parse(self, response):
        '''
        Handle login with provided credentials
        '''
        #Ingresa información al formulario de Logueo
        return FormRequest.from_response(
                response,
                formxpath='//form[contains(@action, "login")]',        #etiquetas form que tengan una propiedad action, en dicha propiedad action debe existir la palabra "login"
    ...
    def parse_home(self, response):
        ...
        #Manejar la venta de dispositivo desconocido
        ...
        #Desactivar la autenticación de 2 pasos, sino les generará un response asi: response == "https://mbasic.facebook.com/checkpoint/":
        
        #Si el response ya incluye el nombre de la pagina => ya no lo añado
        #if response.url.find(self.page) != -1:
        #    href = response                     #https://mbasic.facebook.com/<Pagina>
        #else:
        href = response.urljoin(self.page)
        
        self.logger.info('Scraping facebook page {}'.format(href))
        return scrapy.Request(url=href,callback=self.parse_page,meta={'index':1})  #donde lo envio, que funcion va a procesar el request, 
...
#        open_in_browser(response)
        ...
        for post in response.xpath("//div[contains(@data-ft,'top_level_post_id')]"):     
            ...
            self.logger.info(many_features)
            date = []
            ...
            new.add_xpath('url', ".//a[contains(@href,'footer')]/@href")   #Historia completa 
            #page_url #new.add_value('url',response.url)
            
            #returns full post-link in a list
            post = post.xpath(".//a[contains(@href,'footer')]/@href").extract() #Historia completa 
        ...
        #No hay nuevas pagina del mismo año
        if not new_page: 
            self.logger.info('[!] "more" link not found, will look for a "year" link')
            #self.k is the year link that we look for 
            if response.meta['flag'] == self.k and self.k >= self.year:                                     #  self.year es el año de restricción (ej: solo pub a partir del 2018)
                xpath = "//div/a[contains(@href,'time') and contains(text(),'" + str(self.k) + "')]/@href"  #Link 2018, 2017, 2016...
                new_page = response.xpath(xpath).extract()
                #Si existen links de años anteriores
                if new_page:
        ...
        else: #hay nuevas paginas del mismo año
            new_page = response.urljoin(new_page[0])
            if 'flag' in response.meta: #A partir de la 2da llamada recursiva
            ...
            else:  #Primera llamada recursiva del metodo parse_page
